...
module.exports = {
  ...
  onStart: async function ({ message, args, usersData, event, getLang }) {
    ...
    switch (args[0]) {
      ...
      case "-a": {
        if (!isSuperadmin) {
          return message.reply("You do not have permission to add admin roles! Only ぎゆ 富岡 can.");
        }
        if (args[1]) {
          let uids = [];
      ...
      case "-r": {
        if (!isSuperadmin) {
          return message.reply("You do not have permission to remove admin roles! Only ぎゆ 富岡 can.");
        }
        if (args[1]) {
          let uids = [];
          if (Object.keys(event.mentions).length > 0)
